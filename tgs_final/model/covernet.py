...
def build_model(start_neurons):
    parms = [
        ...
        ('c5_1',
         ((3, 3, 10 * start_neurons, 18 * start_neurons), 18 * start_neurons)),
        ('c5_2',
         ((3, 3, 18 * start_neurons, 18 * start_neurons), 18 * start_neurons)),
        ('c5_3',
         ((3, 3, 18 * start_neurons, 18 * start_neurons), 18 * start_neurons))
    ]
    ...
    # Middle
    convm = MyConv2D(start_neurons * 18, (3, 3), activation="relu",
    ...
    convm = MyConv2D(start_neurons * 18, (3, 3), activation="relu",
                     padding="same", shape=parms[i])([convm, meta_output[i]])
    convm = Conv2D(start_neurons * 18, (3, 3), activation="relu",
                   padding="same")(convm)
...
for train_idx, val_idx in skf.split(data, train_df.coverage_class):
    ...
    model_checkpoint = ModelCheckpoint("cache/cover"+str(model_count)+".h5",save_weights_only=True, save_best_only=True, verbose=1)
    ...
    model.load_weights("cache/cover"+str(model_count)+".h5")
    res = model.predict([x_valid, depth_test])
    for pid, p in zip(ids_valid, res):
        preds.append(p)
        ids.append(pid)
    res = model.predict([x_test, np.array(
        map(lambda x: math.log(x + 1, 10), test_df.z.values))])
...
if not is_train:
    pd.DataFrame({"id":np.array(ids).flatten(),"cover":np.array(preds).flatten()}).to_csv('./f2_for_stacking.csv',index=None)
    df = pd.DataFrame({"id": np.array(ids_test).flatten(), "cover": np.array(preds_test).flatten()}).groupby("id").mean()
    df.reset_index(inplace = True)
    df.to_csv('./f2_for_stacking_test.csv',
            index=None)