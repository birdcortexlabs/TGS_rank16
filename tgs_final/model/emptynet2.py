...
train_df["images"] = [np.array(load_img("../input/train/images/{}.png".format(idx), grayscale=True)) / 25.50  for idx in
...
x_test = np.array(
        [upsample(np.array(load_img("../input/test/images/{}.png".format(idx),
                                    grayscale=True)) / 25.5) for idx in
...
print(train_df.isempty.value_counts())
...
def build_model(start_neurons):
    parms = [
        # ('c1_1', ((3, 3, 1, start_neurons), start_neurons)),
        #      ('c1_2', ((3, 3, start_neurons, start_neurons), start_neurons)),
        #      ('c2_1', ((3, 3, start_neurons, 2*start_neurons), 2*start_neurons)),
        #      ('c2_2', ((3, 3, 2*start_neurons, 2*start_neurons), 2*start_neurons)),
        #      ('c3_1', ((3, 3, 2*start_neurons, 4*start_neurons), 4*start_neurons)),
        #      ('c3_2', ((3, 3, 4*start_neurons, 4*start_neurons), 4*start_neurons)),
        #      ('c4_1', ((3, 3, 4*start_neurons, 8*start_neurons), 8*start_neurons)),
        #      ('c4_2', ((3, 3, 8*start_neurons, 8*start_neurons), 8*start_neurons)),
        ('c5_1',
         ((3, 3, 8 * start_neurons, 16 * start_neurons), 16 * start_neurons)),
        ('c5_2',
    ...
    x = xc1
    hidden = concatenate(
        [Dense(64, activation='relu')(x), Dense(8, activation='sigmoid')(x2),
         Dense(8, activation='relu')(x2)])
    hidden = Dropout(0.05)(hidden)
    meta_output = []
    for name, shape in parms:
        hidden2 = concatenate([Dense(32, activation='sigmoid')(x),
                               Dense(32, activation='sigmoid')(x2),
    ...
    i = 0
    # 128 -> 64
    conv1 = Conv2D(start_neurons * 4, (7, 7), activation="relu",
                   padding="same")(input_layer)
    # conv1 = BatchNormalization()(conv1)
    conv1 = Conv2D(start_neurons * 4, (3, 3), activation="relu",
                   padding="same")(conv1)
    # conv1 = BatchNormalization()(conv1)
    # conv1 = Conv2DInfer(activation='relu')([conv1, concatenate([hidden,Mean_std(conv1)])])
    ...
    # 32 -> 16
    conv3 = Conv2D(start_neurons * 4, (3, 3), activation="relu",
                   padding="same")(pool2)
    ...
    # 16 -> 8
    conv4 = Conv2D(start_neurons * 8, (3, 3), activation="relu",
                   padding="same")(pool3)
    ...
    # convm = Conv2DInfer(activation='sigmoid')([convm, hidden])
    i += 1
    convm = MyConv2D(start_neurons * 16, (3, 3), activation="relu",
    ...
    # convm = MyConv2D(start_neurons * 16, (3, 3), activation="relu",
    #                  padding="same", shape=parms[i])([convm, meta_output[i]])
    # convm = Conv2D(start_neurons * 8, (3, 3), activation="relu",
    #                padding="same")(convm)
    convm = Conv2D(start_neurons * 8, (3, 3), activation="relu",
                   padding="same")(convm)

    x = Dense(32)(concatenate([Flatten()(convm),Mean_std()(convm)]))
    x = PReLU()(x)
    x = Dropout(0.3)(x)
    output_isempty = Dense(1,activation='sigmoid',name = 'isempty_output')(x)

    model = Model([input_layer,input2], output_isempty)
    return model
...
for train_idx, val_idx in skf.split(data, labels):
    ...
    sample_weight += [0.8] * len(x_train_org)

    # x_train = np.append(x_train, [np.flipud(x) for x in x_train_org], axis=0)
    # y_train = np.append(y_train, y_train_org, axis=0)
    ...
    model_checkpoint = ModelCheckpoint("cache/empty2_"+str(model_count)+".model",save_weights_only=True, save_best_only=True, verbose=1)
    reduce_lr = ReduceLROnPlateau(factor=0.15, patience=15, min_lr=0.00005, verbose=1)
    epochs = 60
    batch_size = 32
    if is_train:
    ...
    model.load_weights("cache/empty2_"+str(model_count)+".model")
    res = model.predict([x_valid,depth_test])
    for pid, p in zip(ids_valid, res):
        preds.append(p)
        ids.append(pid)
    res = model.predict([x_test,np.array(map(lambda x: math.log(x + 1, 10), test_df.z.values))])
...
if not is_train:
    pd.DataFrame({"id":np.array(ids).flatten(),"isempty":np.array(preds).flatten()}).to_csv('./f1_for_stacking.csv',index=None)
    df = pd.DataFrame({"id": np.array(ids_test).flatten(), "isempty": np.array(preds_test).flatten()}).groupby("id").mean()
    df.reset_index(inplace = True)
    df.to_csv('./f1_for_stacking_test.csv',
            index=None)
