...
def build_model(start_neurons):
    ...
    for name, shape in parms:
        ...
        meta_output.append(concatenate([Lambda(lambda x: 0.05 * x)(kernel),
    ...
    def xception_unit(x, num):
        ...
        res.append(Conv2D(num, (3, 3), activation="relu",
                          padding="same")(x))
        res.append(Conv2D(num, (1, 1), activation="relu",
                          padding="same")(Conv2D(num, (1, 3), activation="relu",
                                                 padding="same")(
            Conv2D(num, (3, 1), activation="relu",
                   padding="same")(x))))
...
for train_idx, val_idx in skf.split(images, labels):
    ...
    modelname = 'cache/inception3_'+str(model_count) + '.h5'
    ...
    modelname2 = 'cache/inception3lov_' + str(model_count) + '.h5'
    modelname3 = 'cache/inception3mse_' + str(model_count) + '.h5'
    ...
    preds_test.astype(np.float32).tofile("cache/inception3lov_" + str(model_count) + "_valid_np")
    ...
    preds_test.tofile("cache/inception3lov_" + str(model_count) + "_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    ...
    preds_test.astype(np.float32).tofile("cache/inception3mse_" + str(model_count) + "_valid_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    ...
    preds_test = np.array([downsample(x) for x in preds_test], dtype=np.float32)
    preds_test.tofile("cache/inception3mse_" + str(model_count) + "_np")
