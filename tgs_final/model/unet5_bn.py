...
def build_model(start_neurons):
    ...
    middle2 = uconv1
    uconv1 = Dropout(0.3)(uconv1)
    uconv1 = concatenate([Conv2D(16, (1, 1), padding="same", activation="relu")(uconv1),Conv2D(8, (1, 1), padding="same", activation="sigmoid")(uconv1)])
    uconv1 = concatenate([Conv2D(16, (1, 1), padding="same", activation="relu")(uconv1),Conv2D(8, (1, 1), padding="same", activation="sigmoid")(uconv1)])
...
for train_idx, val_idx in skf.split(images, labels):
    ...
    model,model2,pretrain_model = build_model(17)
    ...
    modelname = 'cache/unet5_'+str(model_count) + '.h5'
    ...
    modelname2 = 'cache/unet5lov_' + str(model_count) + '.h5'
    modelname3 = 'cache/unet5mse_' + str(model_count) + '.h5'
    ...
    preds_test = (res[:length] + np.array(
        [np.fliplr(x) for x in res[length:2 * length]]) * 0.6) / 1.6
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    preds_test.astype(np.float32).tofile("cache/unet5lov_" + str(model_count) + "_valid_np")
    ...
    preds_test.tofile("cache/unet5lov_" + str(model_count) + "_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    ...
    preds_test = (res[:length] + np.array(
        [np.fliplr(x) for x in res[length:2 * length]]) * 0.6) / 1.6
    preds_test.astype(np.float32).tofile("cache/unet5mse_" + str(model_count) + "_valid_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    ...
    preds_test = (res[:length] + np.array(
        [np.fliplr(x) for x in res[length:2 * length]]) * 0.6)/1.6
    preds_test = np.array([downsample(x) for x in preds_test], dtype=np.float32)
    preds_test.tofile("cache/unet5mse_" + str(model_count) + "_np")
