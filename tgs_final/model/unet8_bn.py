...
for train_idx, val_idx in skf.split(images, labels):
    ...
    modelname = 'cache/unet8_'+str(model_count) + '.h5'
    ...
    modelname2 = 'cache/unet8lov_' + str(model_count) + '.h5'
    modelname3 = 'cache/unet8mse_' + str(model_count) + '.h5'
    ...
    preds_test.astype(np.float32).tofile("cache/unet8lov_" + str(model_count) + "_valid_np")
    ...
    preds_test.tofile("cache/unet8lov_" + str(model_count) + "_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    ...
    preds_test.astype(np.float32).tofile("cache/unet8mse_" + str(model_count) + "_valid_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    ...
    preds_test = np.array([downsample(x) for x in preds_test], dtype=np.float32)
    preds_test.tofile("cache/unet8mse_" + str(model_count) + "_np")
