...
img_size_ori = 101
img_size_target = 128 + 32 + 32
img_size_ori = img_size_target
padsize1 = 13 + 32
padsize2 = 14 + 32

def mycustom(y_true, y_pred):
    y_true = y_true[:,padsize1:-padsize2,padsize1:-padsize2,:]
    y_pred = y_pred[:,padsize1:-padsize2, padsize1:-padsize2, :]
    y_true = K.batch_flatten(y_true)
    y_pred = K.batch_flatten(y_pred)
    y_union = K.clip(y_true+y_pred,1e-5,1)
    y_weight = K.clip(101 * 101 * 1.0 / K.sum(y_union, axis=-1), 1.0, 80.0)
    return K.mean(K.square(K.abs(y_pred - y_true)), axis=-1) * y_weight
...
def padding1(img):
    ...
    return np.pad(img,((padsize1,padsize2),(padsize1,padsize2)),mode ='constant',constant_values=((0,0),(0,0)))
...
train_df["images"] = [(imagetran(np.array(
    load_img("../input/aug_images/{}.png".format(idx),
             grayscale=True)) / 255.0)) for idx in
                      tqdm_notebook(train_df.index)]

# train_df["images2"] = [np.array(
#     load_img("../input/aug_masks/{}.png".format(idx), grayscale=True)) / 255.0
#                      for idx in
#                      tqdm_notebook(train_df.index)]
train_df["masks"] = [np.array(load_img("../input/train/masks/{}.png".format(idx), grayscale=True)) / 255.0 for idx in
                     tqdm_notebook(train_df.index)]
train_df["weights"] = train_df.masks.map(lambda x:1.0)
train_df["coverage"] = train_df.masks.map(np.sum) / pow(img_size_ori, 2)

y_org = np.array(train_df["masks"].values.tolist()).reshape(-1, 101, 101, 1)
train_df["z"] = train_df["z"].map(lambda x:math.log(x+1,10))
test_df["z"] = test_df["z"].map(lambda x:math.log(x+1,10))
x_test = np.array(
    [(imagetran(np.array(load_img("../input/aug_images/{}.png".format(idx),
                                grayscale=True)) / 255.0)) for idx in
     tqdm_notebook(test_df.index)]).reshape(-1, img_size_target,
                                            img_size_target, 1)
# x_test2 = np.array(
#     [(imagetran(np.array(load_img("../input/aug_masks/{}.png".format(idx),
#                                 grayscale=True)) / 255.0)) for idx in
#      tqdm_notebook(test_df.index)]).reshape(-1, img_size_target,
#                                             img_size_target, 1)
...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        thres = self.thre
        length = self.X_val[0].shape[0] / 2

        preds_valid = self.model.predict(self.X_val)
        preds_valid = np.array([downsample(x) for x in preds_valid])
        y_valid = np.array(self.y_val)
        iou2 = iou_metric_batch(np.int32(y_valid > 0.5), np.int32(preds_valid[:length][:,padsize1:-padsize2,padsize1:-padsize2,:] > thres),0.005)
        ...
        iou = iou_metric_batch(np.int32(y_valid > 0.5), np.int32(preds_valid2[:,padsize1:-padsize2,padsize1:-padsize2,:] > thres),0.005)
        logs['iou'] = iou
        print('val iou:{},iouorg:{},mean:{}'.format(iou,iou2,np.mean(preds_valid[:,padsize1:-padsize2,padsize1:-padsize2,:])))
        del preds_valid
        gc.collect()
...
masks = np.array(train_df.masks.map(padding1).tolist()).reshape(-1, img_size_target, img_size_target, 1)
labels = train_df.coverage_class

for train_idx_t, val_idx_t in skf.split(images, labels):
    print(model_count)
...
if True:
    ...
    x_train = np.append(x_train, [np.fliplr(x) for x in x_train_org], axis=0).astype(np.float32)
    y_train = np.append(y_train, [np.fliplr(x) for x in y_train_org], axis=0).astype(np.float32)
    ...
    x_valid2 = np.append(x_valid, [np.fliplr(x) for x in x_valid_org], axis=0).astype(np.float32)
    y_valid2 = np.append(y_valid, [np.fliplr(x) for x in y_valid_org_temp], axis=0).astype(np.float32)
    depth_valid2 = np.append(depth_valid, np.array(map(lambda x: x, depth_valid_org)), axis=0).astype(np.float32)

    lh = LossHistory([x_valid2,depth_valid2], y_valid_org, model)
    early_stopping = EarlyStopping(patience=10, verbose=1, mode='max',monitor='iou')
    modelname = 'cache/unet11_'+str(model_count) + '.h5'
    ...
    history = model.fit([x_train, depth_train], y_train,
                        ...
                        sample_weight=np.array(sample_weight),
                        callbacks=[lh,early_stopping, reduce_lr, model_checkpoint])
    model.load_weights(modelname)
    ...
    modelname2 = 'cache/unet11lov_' + str(model_count) + '.h5'
    modelname3 = 'cache/unet11mse_' + str(model_count) + '.h5'

    model.load_weights(modelname)
    model2.compile(loss=mycustom2, optimizer=RMSprop(lr=0.0005))
    reduce_lr = ReduceLROnPlateau(factor=0.15, patience=5, min_lr=0.00005,
                                  verbose=1)
    ...
    history = model2.fit([x_train, depth_train], y_train,
                         ...
                         batch_size=batch_size,
                         sample_weight=np.array(sample_weight),
                         callbacks=[lh, early_stopping, reduce_lr,
                                    model_checkpoint])
    model.load_weights(modelname)
    ...
    history = model.fit([x_train, depth_train], y_train,
                         ...
                         callbacks=[lh, early_stopping, reduce_lr,
                                    model_checkpoint])
    del x_train, depth_train, y_train
    ...
    preds_test = (res[:length] + np.array(
        [np.fliplr(x) for x in res[length:2 * length]]) * 0.6
                  ) / 1.6
    preds_test = preds_test[:,padsize1:-padsize2, padsize1:-padsize2, :]
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    preds_test.astype(np.float32).tofile("cache/unet11lov_" + str(model_count) + "_valid_np")
    del preds_test
    ...
    preds_test.tofile("cache/unet11lov_" + str(model_count) + "_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    del preds_test
    ...
    preds_test = preds_test[:, padsize1:-padsize2, padsize1:-padsize2, :]
    preds_test.astype(np.float32).tofile("cache/unet11mse_" + str(model_count) + "_valid_np")
    print iou_metric_batch(np.int32(y_valid_org > 0.5), np.int32(preds_test > index_best[0]), index_best[1])
    del preds_test
    ...
    preds_test = np.array([downsample(x) for x in preds_test], dtype=np.float32)
    preds_test = preds_test[:, padsize1:-padsize2, padsize1:-padsize2, :]
    preds_test.tofile("cache/unet11mse_" + str(model_count) + "_np")
